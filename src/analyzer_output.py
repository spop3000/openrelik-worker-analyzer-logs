...
class AnalyzerOutput:
    ...
    def validate(self):
        ...
        if not self.analyzer_identifier:
            raise Exception("analyzer identifier is empty")

        if not self.analyzer_name:
            raise Exception("analyzer name is empty")

        if self.result_status.lower() not in ["success", "failed"]:
            raise Exception("unknown result status {self.result_status}")

        if self.result_priority.upper() not in [
            "CRITICAL",
            "HIGH",
            "MEDIUM",
            "LOW",
            ...
            raise Exception("unknown result priority valude {self.result_priority}")

        if not self.result_summary:
            raise Exception("result summary is empty")
